...
def main():
    ...
    # --- DATATYPE FIX & DATA SPLITTING ---
    if "student_id" in cleaned_df.columns:
        # Fixed: .astype(str) ensures numbers and strings in student_id do not cause a TypeError during sorting
        cleaned_df["student_id"] = cleaned_df["student_id"].fillna("unknown_user").astype(str)
        ...
        # Leaky features to isolate away from the mathematical training pipeline
        columns_to_drop = [
